...
namespace OpenLogReplicator {
    ...
    bool Parser::trySkipUntrackedPair(const uint8_t* data, uint32_t recordSize, uint32_t& offset, const LwnMember* lwnMember,
        ...
        // Two undo kinds are safe to decide from the head alone, because no decoder of their
        // paired redo vector writes obj/dataObj, so the fallback to redoLogRecord2 in
        // appendToTransaction / appendToTransactionIndex always yields 0:
        //   0x0B01 row DML, paired with 0x0Bxx, decided by checkTableDict(obj)
        //   0x0A16 index maintenance, paired with 0x0Axx, decided by checkLobIndexDict(dataObj)
        // For opc 0x1A01 the body (kdli) overwrites obj/dataObj (OpCode.h kdliSuplog /
        // kdliFpload), so the head's object is not final and it is never skipped here.
        const bool rowDml = (redoLogRecord1->opc == 0x0B01);
        const bool index = (redoLogRecord1->opc == 0x0A16);
        if (!rowDml && !index) {
            if (redoLogRecord1->opc == 0x1A01)
                ++keptLobUndo;
            else
                ++keptOtherOpc;
            return false;
        ...
        // The redo vector must be of the matching family (0x1A02 LOB index is 0x1Axx and is
        // therefore excluded here, as appendToTransactionIndex treats it differently)
        if ((rowDml && (peek.opCode & 0xFF00) != 0x0B00) || (index && (peek.opCode & 0xFF00) != 0x0A00)) {
            ++keptNoPartner;
            return false;
        }
        ...
        // Skip other PDB vectors (mirrors appendToTransaction / appendToTransactionIndex)
            ...
            if (transaction == nullptr) {
                ...
                if (rowDml) {
                    ...
                    if (table == nullptr && !ctx->isFlagSet(Ctx::REDO_FLAGS::SCHEMALESS))
                        skip = true;
                    else if (table != nullptr && (DbTable::isSystemTable(table->options) || DbTable::isSchemaTable(table->options)))
                        ++keptDictTable;
                    else
                        ++keptUserTable;
                } else {
                    // appendToTransactionIndex: dataObj from the undo, else from the redo (always 0);
                    // a pair is kept only when it belongs to a tracked LOB index
                    const typeDataObj dataObj = (redoLogRecord1->dataObj != 0) ? redoLogRecord1->dataObj : 0;
                    const DbLob* lob;
                    ctx->parserThread->contextSet(Thread::CONTEXT::TRAN);
                    {
                        std::unique_lock const lckTransaction(metadata->mtxTransaction);
    ...
    Reader::REDO_CODE Parser::parse() {
        ...
        if (ctx->isTraceSet(Ctx::TRACE::PERFORMANCE)) {
            ...
            if (fastFilter)
                ctx->logTrace(Ctx::TRACE::PERFORMANCE, "Fast filter: " + std::to_string(skippedPairs) + " DML vector pairs, " +
                              std::to_string(skippedIndexPairs) + " index vector pairs skipped; kept: user table " +
