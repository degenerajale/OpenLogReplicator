...
A zone name resolves the offset per wall-clock hour, so DST is handled automatically; the offset is cached per hour and costs nothing measurable.
The zone name must exist under `/usr/share/zoneinfo` on the machine running OpenLogReplicator (install `tzdata` in the container image), otherwise the configuration is rejected.
