...
* `0x0400` — Track transactions whose begin was not read (already open at start): log warning 60011 at their commit and, for JSON output, emit a `"p"` (partial commit) marker with the commit position and xid. Their content is never emitted.
