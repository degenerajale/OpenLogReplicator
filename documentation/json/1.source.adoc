...
* `0x80000` — Fast filter: skip decoding DML change vectors for objects outside the
  configured tables. Reduces CPU when few tables are replicated. Ignored in schemaless
  mode and when redo dumping is enabled. Output is identical with and without it.
...
`read-buffer-max-mb` must be at least `read-parallel + 2`.
